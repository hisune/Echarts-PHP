...
{            
    ...
    /**
     * @var int 指定 visualMapContinuous 组件的允许的最小值。min 必须用户指定。[visualMap.min, visualMax.max] 形成了视觉映射的『定义域』。
     */
    public $min;        
        
    /**
     * @var int 指定 visualMapContinuous 组件的允许的最大值。max 必须用户指定。[visualMap.min, visualMax.max] 形成了视觉映射的『定义域』。
     */
    public $max;        
        
    /**
     * @var array 指定手柄对应数值的位置。range 应在 min max 范围内。例如：
     * chart.setOption({
     *     visualMap: {
     *         min: 0,
     *         max: 100,
    ...
    /**
     * @var boolean 是否显示拖拽用的手柄（手柄能拖拽调整选中范围）。
    ...
    /**
     * @var int 数据展示的小数精度，默认为0，无小数点。
     */
    public $precision = 0;        
        
    /**
     * @var int 图形的宽度，即长条的宽度。
     */
    public $itemWidth = 20;        
        
    /**
     * @var int 图形的高度，即长条的高度。
     */
    public $itemHeight = 140;        
        
    /**
     * @var string 指定组件中手柄和文字的摆放位置，可选值为：
     * 
     * auto 自动决定。
    ...
    /**
     * @var array 两端的文本，如 [High, Low]。参见例子。
     * text 中的顺序，其实试试就知道。若要看详细的规则，参见 visualMap.inverse。
     */
    public $text;        
        
    /**
     * @var array 两端文字主体之间的距离，单位为px。参见 visualMap-continuous.text
     */
    public $textGap = '10';        
        
    /**
     * @var boolean 是否显示 visualMap-continuous 组件。如果设置为 false，不会显示，但是数据映射的功能还存在。
     */
    public $show = true;        
        
    /**
     * @var int 指定用数据的『哪个维度』，映射到视觉元素上。『数据』即 series.data。
    ...
    /**
     * @var int|array 指定取哪个系列的数据，即哪个系列的 series.data。
     * 默认取所有系列。
     */
    public $seriesIndex;        
        
    /**
     * @var boolean 打开 hoverLink 功能时，鼠标悬浮到 visualMap 组件上时，鼠标位置对应的数值 在 图表中对应的图形元素，会高亮。
     * 反之，鼠标悬浮到图表中的图形元素上时，在 visualMap 组件的相应位置会有三角提示其所对应的数值。
     */
    public $hoverLink = true;        
        
    /**
     * @var array 定义 在选中范围中 的视觉元素。（用户可以和 visualMap 组件交互，用鼠标或触摸选择范围）
     * 可选的视觉元素有：
     * 
     * symbol: 图元的图形类别。
     * symbolSize: 图元的大小。
    ...
    /**
     * @var array 定义 在选中范围外 的视觉元素。（用户可以和 visualMap 组件交互，用鼠标或触摸选择范围）
     * 可选的视觉元素有：
     * 
     * symbol: 图元的图形类别。
     * symbolSize: 图元的大小。
    ...
    /**
     * @var VisualMap\Controller visualMap 组件中，控制器 的 inRange outOfRange 设置。如果没有这个 controller 设置，控制器 会使用外层的 inRange outOfRange 设置；如果有这个 controller 设置，则会采用这个设置。适用于一些控制器视觉效果需要特殊定制或调整的场景。
     */
    public $controller;        
    ...
    /**
     * @var string|int visualMap 组件离容器左侧的距离。
    ...
    /**
     * @var string|int visualMap 组件离容器上侧的距离。
    ...
    /**
     * @var string|int visualMap 组件离容器右侧的距离。
     * right 的值可以是像 20 这样的具体像素值，可以是像 20% 这样相对于容器高宽的百分比。
     */
    public $right = 'auto';        
        
    /**
     * @var string|int visualMap 组件离容器下侧的距离。
     * bottom 的值可以是像 20 这样的具体像素值，可以是像 20% 这样相对于容器高宽的百分比。
     */
    public $bottom = 0;        
        
    /**
     * @var string 如何放置 visualMap 组件，水平（horizontal）或者竖直（vertical）。
     */
    public $orient = 'vertical';        
        
    /**
     * @var int|array visualMap-continuous内边距，单位px，默认各方向内边距为5，接受数组分别设定上右下左边距。
     * 使用示例：
     * // 设置内边距为 5
     * padding: 5
    ...
    /**
     * @var array 这个配置项，是为了兼容 ECharts2 而存在，ECharts3 中已经不推荐使用。它的功能已经移到了 visualMap-continuous.inRange 和 visualMap-continuous.outOfRange 中。
    ...
    /**
     * @var VisualMap\TextStyle 
     */
    public $textStyle;        
        
    /**
     * @var string|callable 标签的格式化工具。
     * 
    ...
    /**
     * @var int 对于连续型数据，自动平均切分成几段。默认为5段。
    ...
    /**
     * @var array 自定义『分段式视觉映射组件（visualMapPiecewise）』的每一段的范围，以及每一段的文字，以及每一段的特别的样式。例如：
     * pieces: [
    ...
    /**
     * @var array 用于表示离散型数据（或可以称为类别型数据、枚举型数据）的全集。
    ...
    /**
     * @var boolean 当 type 为 piecewise 且使用 min/max/splitNumber 时，此参数有效。当值为 true 时，界面上会额外多出一个『&lt; min』的选块。
     */
    public $minOpen;        
        
    /**
     * @var boolean 当 type 为 piecewise 且使用 min/max/splitNumber 时，此参数有效。当值为 true 时，界面上会额外多出一个『&gt; max』的选块。
     */
    public $maxOpen;        
    ...
    /**
     * @var boolean 是否显示每项的文本标签。默认情况是，如果 visualMap-piecewise.text 被使用则不显示文本标签，否则显示。
     */
    public $showLabel;        
        
    /**
     * @var int 每两个图元之间的间隔距离，单位为px。
     */
    public $itemGap = 10;        
        
    /**
     * @var string 默认的图形。可选值为：
     * circle, rect, roundRect, triangle, diamond, pin, arrow
     * 。
